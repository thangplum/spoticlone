...
interface PlaylistFuncProps {
    type?: string;
    follow?: boolean;
    onFollow?: () => void;
    setMessage?: (message: string) => void;
    setPlay?: () => void;
    playContext?: (trackURI: string) => void;
    loggedIn?: boolean;
}
...
const PlayButtonLarge: React.FC<PlayButtonProps>= ({loggedIn, playContext}) => {
...
const LikeButton: React.FC<LikeButtonProps> = ({follow, onFollow, loggedIn}) => {
    if (loggedIn){
        return (
            <button className={`likeButton ${follow? 'noHover':''} no-outline`} style={{color: follow? 'var(--spotify-green)': 'none'}} title={follow? 'Remove from Library':"Save to Your Library"} onClick={onFollow}>
...
const FollowButton: React.FC<LikeButtonProps> = ({follow, onFollow, loggedIn}) => {
    if (loggedIn){
        return (
...
export const PlaylistFunc: React.FC<PlaylistFuncProps> = ({ type, follow, onFollow, setMessage, setPlay, playContext }) => {
    ...
    switch(type) {
        ...
        case 'none':
            return (
                <div className="playListFunctions">
                    <MoreButton onClick={() => {if(setMessage) setMessage('Oops, it look like I chose not to implement this feature :)')}}/>
                </div>
            )
        case 'user':
            return (
